...
this.getCurrentMessageState = function (game, channel)
{
  ...
  // if(doEndGame)
  // {
  //   this.stopGame(game, channel, false); <<<<<DEAL WITH THAT
  // }
}

this.initialiseReactions = function(message)
{
  // Set up reactions.
  for(var i = 0; i < 9; i++)
  {
    message.react(info.tictactoeArrows[i]);
  }
}

this.makeBotMove = function(game, channel) {}

this.updateMessage = function(game, channel, init=false)
{
  msg = this.getCurrentMessageState(game, channel);

  if(init)
  {
    msg = channel.send(msg).then(sentMessage => {
      gameList[game].gameMessage = sentMessage;
      this.initialiseReactions(sentMessage);
    });
  }
  else
  {
    var end = gameList[game].gameEnd
    gameList[game].gameMessage.edit(msg).then(sentMessage => {
      if(!end && gameList[game].botDiff > 0
      && gameList[game].turnNumber%2==1) this.makeBotMove(game, channel);
    });

    if(gameList[game].gameEnd)
    {
      this.stopGame(game);
    }
  }
...
this.startNewGame = function(message)
{
    ...
    else
    {
      ...
      var botDiff = 0; // default is bot not playing
      if(message.mentions.users.first().bot)
      {
        messageArg3 = helper.getMessageAsArgs(message)[3];
        console.log("messageArg3:" + messageArg3)
        if(messageArg3 === undefined || messageArg3 === "medium")
        {
          // if they didnt say shit, hit em with the random
          botDiff = 2;
        }
        else if(messageArg3 === "random")
        {
          botDiff = 1;
        }
      }

      if(botDiff == 0 && Math.random() < 0.5)
      {
        a = users.shift();
        users[1] = a;
      }
      ...
      this.updateMessage(this.getGame(message), message.channel, true);

      // this.sendCurrentTurn(this.getGame(message), message.channel);
    }
}

this.updateBoard = function(move, game, channel)
{
  gameList[game].board[Math.floor(move/3)][move%3] = gameList[game].turnNumber%2 == 0 ? 1 : -1;
  gameList[game].turnNumber++;
  this.updateMessage(game, channel);
}

this.getMove = function(direction, line, b)
{
  for(var i = 0; i < boardSize; i++)
  {
    // console.log("In order, the returned moves: (direction = " + direction + ", i = " + i + ")");
    // console.log("b[" + i + "][" + line + "] = " + b[i][line] + ": " + i*boardSize + line);
    // console.log("b[" + line + "][" + i + "] = " + b[line][i] + ": " + line*boardSize + i);
    // console.log("b[" + i + "][" + (line === 0 ? i : 2 - i) + "] = " + b[i][line === 0 ? i : 2 - i] + ": " + i*boardSize + (line === 0 ? i : 2 - i));
    if(direction === 0 && b[i][line] === 0)      return i*boardSize + line;
    else if(direction === 2 && b[line][i] === 0) return line*boardSize + i;
    else if(direction === 1 && b[i][line === 0 ? i : 2 - i] === 0) return i*boardSize + (line === 0 ? i : 2 - i);

    // switch(direction)
    // {
    //   case(0): // row
    //     if(b[i][line] == 0) return line*boardSize + i;
    //     break;
    //   case(2): // col
    //     if(b[line][i] == 0) return i*boardSize + line
    //     break;
...
this.botPlacement = function(game, player) // player = 1 means looking for X (user) and -1 means O (bot)
{
  b = gameList[game].board;

  for(var i = 0; i < boardSize; i++)
  {
    if(b[0][i] + b[1][i] + b[2][i] == player*2) return this.getMove(0, i, b)

    else if(b[i][0] + b[i][1] + b[i][2] == player*2) return this.getMove(2, i, b)
  }

  if(b[0][0] + b[1][1] + b[2][2] == player*2) return this.getMove(1, 0, b);
  else if(b[2][0] + b[1][1] + b[0][2] == player*2) return this.getMove(1, 1, b);
...
this.makeBotMove = function(game, channel)
{
  var availableSquares = gameList[game].availableSquares;
  var move = -1;
  var botDiff = gameList[game].botDiff;

  if(botDiff === 0 || !gameList[game].turnNumber%2==1) return;
  else if(botDiff == 2)
  {
    // If we're somehow able to win this turn, uh do it
    move = this.botPlacement(game, -1);

    // Just block them if you can, otherwise random.
    if(move == -1) move = this.botPlacement(game, 1);
  }

  if(move == -1) move = availableSquares[Math.floor(Math.random() * availableSquares.length)];

  this.updateBoard(move, game, channel);
}
...
this.onReaction = function(reaction, user)
{
  ...
  for(var i = 0; i < gameList.length; i++)
  {
    if(gameList[i].gameMessage.id === reaction.message.id)
    {
